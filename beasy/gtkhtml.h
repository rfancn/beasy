/**
 * @file gtkimhtml.h GTK+ IM/HTML rendering component
 * @ingroup beasy
 * @see @ref gtkimhtml-signals
 */
#ifndef _BEASYIMHTML_H_
#define _BEASYIMHTML_H_

#include <gdk/gdk.h>
#include <gtk/gtktextview.h>
#include <gtk/gtktooltips.h>
#include <gtk/gtkimage.h>

#ifdef __cplusplus
extern "C" {
#endif

/**************************************************************************
 * @name Structures
 **************************************************************************/
/*@{*/

#define GTK_TYPE_HTML            (gtk_html_get_type ())
#define GTK_HTML(obj)            (GTK_CHECK_CAST ((obj), GTK_TYPE_HTML, GtkHtml))
#define GTK_HTML_CLASS(klass)    (GTK_CHECK_CLASS_CAST ((klass), GTK_TYPE_HTML, GtkHtmlClass))
#define GTK_IS_HTML(obj)         (GTK_CHECK_TYPE ((obj), GTK_TYPE_HTML))
#define GTK_IS_HTML_CLASS(klass) (GTK_CHECK_CLASS_TYPE ((klass), GTK_TYPE_HTML))
#define GTK_HTML_SCALABLE(obj)   ((GtkHtmlScalable *)obj)
#define GTK_HTML_ANIMATION(obj)   ((GtkHtmlAnimation *)obj)

typedef struct _GtkHtml				GtkHtml;
typedef struct _GtkHtmlClass		GtkHtmlClass;
typedef struct _GtkHtmlFontDetail	GtkHtmlFontDetail;	/* The five elements contained in a FONT tag */
typedef struct _GtkSmileyTree		GtkSmileyTree;
typedef struct _GtkHtmlSmiley		GtkHtmlSmiley;
typedef struct _GtkHtmlScalable		GtkHtmlScalable;
typedef struct _GtkHtmlImage		GtkHtmlImage;
typedef struct _GtkHtmlAnimation	GtkHtmlAnimation;
typedef struct _GtkHtmlHr			GtkHtmlHr;
typedef struct _GtkHtmlFuncs		GtkHtmlFuncs;

typedef enum {
	GTK_HTML_BOLD =       1 << 0,
	GTK_HTML_ITALIC =     1 << 1,
	GTK_HTML_UNDERLINE =  1 << 2,
	GTK_HTML_GROW =       1 << 3,
	GTK_HTML_SHRINK =     1 << 4,
	GTK_HTML_FACE =       1 << 5,
	GTK_HTML_FORECOLOR =  1 << 6,
	GTK_HTML_BACKCOLOR =  1 << 7,
	GTK_HTML_BACKGROUND = 1 << 8,
	GTK_HTML_LINK =       1 << 9,
	GTK_HTML_IMAGE =      1 << 10,
	GTK_HTML_SMILEY =     1 << 11,
	GTK_HTML_LINKDESC =   1 << 12,
	GTK_HTML_STRIKE =     1 << 13,
	GTK_HTML_ALL =       -1
} GtkHtmlButtons;

typedef enum {
	GTK_HTML_SMILEY_CUSTOM =	1 << 0
} GtkHtmlSmileyFlags;

struct _GtkHtml {
	GtkTextView text_view;
	GtkTextBuffer *text_buffer;
	GdkCursor *hand_cursor;
	GdkCursor *arrow_cursor;
	GdkCursor *text_cursor;
	GHashTable *smiley_data;
	GtkSmileyTree *default_smilies;
	char *protocol_name;
	guint scroll_src;
	GTimer *scroll_time;
	GQueue *animations;
	int num_animations;

	gboolean show_comments;

	GtkWidget *tip_window;
	char *tip;
	guint tip_timer;
	GtkTextTag *prelit_tag;

	GList *scalables;
	GdkRectangle old_rect;

	gchar *search_string;

	gboolean editable;
	GtkHtmlButtons format_functions;
	gboolean wbfo;	/* Whole buffer formatting only. */

	gint insert_offset;

	struct {
		gboolean bold:1;
		gboolean italic:1;
		gboolean underline:1;
		gboolean strike:1;
		gchar *forecolor;
		gchar *backcolor;
		gchar *background;
		gchar *fontface;
		int fontsize;
		GtkTextTag *link;
	} edit;

#ifndef BEASY_DISABLE_DEPRECATED
	/** @deprecated */
	char *clipboard_text_string;
	/** @deprecated */
	char *clipboard_html_string;
#else
	char *depr1;
	char *depr2;
#endif

	GSList *images;
	GtkHtmlFuncs *funcs;
};

struct _GtkHtmlClass {
	GtkTextViewClass parent_class;

	void (*url_clicked)(GtkHtml *, const gchar *);
	void (*buttons_update)(GtkHtml *, GtkHtmlButtons);
	void (*toggle_format)(GtkHtml *, GtkHtmlButtons);
	void (*clear_format)(GtkHtml *);
	void (*update_format)(GtkHtml *);
	gboolean (*message_send)(GtkHtml *);
	void (*undo)(GtkHtml *);
	void (*redo)(GtkHtml *);
};

struct _GtkHtmlFontDetail {
	gushort size;
	gchar *face;
	gchar *fore;
	gchar *back;
	gchar *bg;
	gchar *sml;
	gboolean underline;
	gshort bold;
};

struct _GtkSmileyTree {
	GString *values;
	GtkSmileyTree **children;
	GtkHtmlSmiley *image;
};

struct _GtkHtmlSmiley {
	gchar *smile;
	gchar *file;
	GdkPixbufAnimation *icon;
	gboolean hidden;
	GdkPixbufLoader *loader;
	GSList *anchors;
	GtkHtmlSmileyFlags flags;
	GtkHtml *imhtml;
};

struct _GtkHtmlScalable {
	void (*scale)(struct _GtkHtmlScalable *, int, int);
	void (*add_to)(struct _GtkHtmlScalable *, GtkHtml *, GtkTextIter *);
	void (*free)(struct _GtkHtmlScalable *);
};

struct _GtkHtmlImage {
	GtkHtmlScalable scalable;
	GtkImage *image; /**< Contains the scaled version of this pixbuf. */
	GdkPixbuf *pixbuf; /**< The original pixbuf, before any scaling. */
	GtkTextMark *mark;
	gchar *filename;
	int width;
	int height;
	int id;
	GtkWidget *filesel;
};

struct _GtkHtmlAnimation {
	GtkHtmlImage imhtmlimage;
	GdkPixbufAnimation *anim; /**< The original animation, before any scaling. */
	GdkPixbufAnimationIter *iter;
	guint timer;
};

struct _GtkHtmlHr {
	GtkHtmlScalable scalable;
	GtkWidget *sep;
};

typedef enum {
	GTK_HTML_NO_COLOURS          = 1 << 0,
	GTK_HTML_NO_FONTS            = 1 << 1,
	GTK_HTML_NO_COMMENTS         = 1 << 2, /* Remove */
	GTK_HTML_NO_TITLE            = 1 << 3,
	GTK_HTML_NO_NEWLINE          = 1 << 4,
	GTK_HTML_NO_SIZES            = 1 << 5,
	GTK_HTML_NO_SCROLL           = 1 << 6,
	GTK_HTML_RETURN_LOG          = 1 << 7,
	GTK_HTML_USE_POINTSIZE       = 1 << 8,
	GTK_HTML_NO_FORMATTING       = 1 << 9,
	GTK_HTML_USE_SMOOTHSCROLLING = 1 << 10,
	GTK_HTML_NO_SMILEY           = 1 << 11,
} GtkHtmlOptions;

enum {
	GTK_HTML_DRAG_URL = 0,
	GTK_HTML_DRAG_HTML,
	GTK_HTML_DRAG_UTF8_STRING,
	GTK_HTML_DRAG_COMPOUND_TEXT,
	GTK_HTML_DRAG_STRING,
	GTK_HTML_DRAG_TEXT,
	GTK_HTML_DRAG_NUM
};

#define GTK_HTML_DND_TARGETS	\
	{ "text/uri-list", 0, GTK_HTML_DRAG_URL }, \
	{ "_NETSCAPE_URL", 0, GTK_HTML_DRAG_URL }, \
	{ "text/html", 0, GTK_HTML_DRAG_HTML }, \
	{ "x-url/ftp", 0, GTK_HTML_DRAG_URL }, \
	{ "x-url/http", 0, GTK_HTML_DRAG_URL }, \
	{ "UTF8_STRING", 0, GTK_HTML_DRAG_UTF8_STRING }, \
	{ "COMPOUND_TEXT", 0, GTK_HTML_DRAG_COMPOUND_TEXT }, \
	{ "STRING", 0, GTK_HTML_DRAG_STRING }, \
	{ "text/plain", 0, GTK_HTML_DRAG_TEXT }, \
	{ "TEXT", 0, GTK_HTML_DRAG_TEXT }

typedef gpointer    (*GtkHtmlGetImageFunc)        (int id);
typedef gpointer    (*GtkHtmlGetImageDataFunc)    (gpointer i);
typedef size_t      (*GtkHtmlGetImageSizeFunc)    (gpointer i);
typedef const char *(*GtkHtmlGetImageFilenameFunc)(gpointer i);
typedef void        (*GtkHtmlImageRefFunc)        (int id);
typedef void        (*GtkHtmlImageUnrefFunc)      (int id);

struct _GtkHtmlFuncs {
	GtkHtmlGetImageFunc image_get;
	GtkHtmlGetImageDataFunc image_get_data;
	GtkHtmlGetImageSizeFunc image_get_size;
	GtkHtmlGetImageFilenameFunc image_get_filename;
	GtkHtmlImageRefFunc image_ref;
	GtkHtmlImageUnrefFunc image_unref;
};

/*@}*/

/**************************************************************************
 * @name GTK+ IM/HTML rendering component API
 **************************************************************************/
/*@{*/

/**
 * Returns the GType object for an IM/HTML widget.
 *
 * @return The GType for an IM/HTML widget.
 */
GType gtk_html_get_type(void);

/**
 * Creates and returns a new GTK+ IM/HTML widget.
 *
 * @return The GTK+ IM/HTML widget created.
 */
GtkWidget *gtk_html_new(void *, void *);

/**
 * Returns the smiley object associated with the text.
 *
 * @param imhtml The GTK+ IM/HTML.
 * @param sml    The name of the smiley category.
 * @param text   The text associated with the smiley.
 */

GtkHtmlSmiley *gtk_html_smiley_get(GtkHtml * imhtml,
                                        const gchar * sml, const gchar * text);


/**
 * Associates a smiley with a GTK+ IM/HTML.
 *
 * @param imhtml The GTK+ IM/HTML.
 * @param sml    The name of the smiley category.
 * @param smiley The GtkIMSmiley to associate.
 */
void gtk_html_associate_smiley(GtkHtml *imhtml, const gchar *sml, GtkHtmlSmiley *smiley);

/**
 * Removes all smileys associated with a GTK+ IM/HTML.
 *
 * @param imhtml The GTK+ IM/HTML.
 */
void gtk_html_remove_smileys(GtkHtml *imhtml);

/**
 * Sets the function callbacks to use with a GTK+ IM/HTML instance.
 *
 * @param imhtml The GTK+ IM/HTML.
 * @param f      The GtkIMHTMLFuncs struct containing the functions to use.
 */
void gtk_html_set_funcs(GtkHtml *imhtml, GtkHtmlFuncs *f);

/**
 * Enables or disables showing the contents of HTML comments in a GTK+ IM/HTML.
 *
 * @param imhtml The GTK+ IM/HTML.
 * @param show   @c TRUE if comments should be shown, or @c FALSE otherwise.
 */
void gtk_html_show_comments(GtkHtml *imhtml, gboolean show);

/**
 * Gets the protocol name associated with this GTK+ IM/HTML.
 *
 * @param imhtml The GTK+ IM/HTML
 */
const char *gtk_html_get_protocol_name(GtkHtml *imhtml);

/**
 * Associates a protocol name with a GTK+ IM/HTML.
 *
 * @param imhtml        The GTK+ IM/HTML.
 * @param protocol_name The protocol name to associate with the IM/HTML.
 */
void gtk_html_set_protocol_name(GtkHtml *imhtml, const gchar *protocol_name);

/**
 * Appends HTML formatted text to a GTK+ IM/HTML.
 *
 * @param imhtml  The GTK+ IM/HTML.
 * @param text    The formatted text to append.
 * @param options A GtkHtmlOptions object indicating insert behavior.
 */
#define gtk_html_append_text(imhtml, text, options) \
 gtk_html_append_text_with_images(imhtml, text, options, NULL)

/**
 * Appends HTML formatted text to a GTK+ IM/HTML.
 *
 * @param imhtml  The GTK+ IM/HTML.
 * @param text    The formatted text to append.
 * @param options A GtkHtmlOptions object indicating insert behavior.
 * @param unused  Use @c NULL value.
 */
void gtk_html_append_text_with_images(GtkHtml *imhtml,
                                         const gchar *text,
                                         GtkHtmlOptions options,
                                         GSList *unused);

/**
 * Inserts HTML formatted text to a GTK+ IM/HTML at a given iter.
 *
 * @param imhtml  The GTK+ IM/HTML.
 * @param text    The formatted text to append.
 * @param options A GtkHtmlOptions object indicating insert behavior.
 * @param iter    A GtkTextIter in the GTK+ IM/HTML at which to insert text.
 */
void gtk_html_insert_html_at_iter(GtkHtml        *imhtml,
                                    const gchar      *text,
                                    GtkHtmlOptions  options,
                                    GtkTextIter      *iter);

/**
 * Scrolls a GTK+ IM/HTML to the end of its contents.
 *
 * @param imhtml  The GTK+ IM/HTML.
 * @param smooth  A boolean indicating if smooth scrolling should be used.
 */
void gtk_html_scroll_to_end(GtkHtml *imhtml, gboolean smooth);

/**
 * Delete the contents of a GTK+ IM/HTML between start and end.
 *
 * @param imhtml  The GTK+ IM/HTML.
 * @param start   a postition in the imhtml's buffer
 * @param end     another postition in the imhtml's buffer
 */
void gtk_html_delete(GtkHtml *imhtml, GtkTextIter *start, GtkTextIter *end);

/**
 * Purges the contents from a GTK+ IM/HTML and resets formatting.
 *
 * @param imhtml  The GTK+ IM/HTML.
 */
#define gtk_imhtml_clear(imhtml) \
 gtk_html_delete(imhtml, NULL, NULL)

/**
 * Scrolls a GTK+ IM/HTML up by one page.
 *
 * @param imhtml  The GTK+ IM/HTML.
 */
void gtk_html_page_up(GtkHtml *imhtml);

/**
 * Scrolls a GTK+ IM/HTML down by one page.
 *
 * @param imhtml  The GTK+ IM/HTML.
 */
void gtk_html_page_down(GtkHtml *imhtml);

/**
 * Creates and returns an new GTK+ IM/HTML scalable object.
 *
 * @return A new IM/HTML Scalable object.
 */
GtkHtmlScalable *gtk_imhtml_scalable_new(void);

/**
 * Creates and returns a new GTK+ IM/HTML scalable object with an image.
 *
 * @param img      A GdkPixbuf of the image to add.
 * @param filename The filename to associate with the image.
 * @param id       The id to associate with the image.
 *
 * @return A new IM/HTML Scalable object with an image.
 */
GtkHtmlScalable *gtk_html_image_new(GdkPixbuf *img, const gchar *filename, int id);

/**
 * Creates and returns a new GTK+ IM/HTML scalable object with an
 * animated image.
 *
 * @param img      A GdkPixbufAnimation of the image to add.
 * @param filename The filename to associate with the image.
 * @param id       The id to associate with the image.
 *
 * @return A new IM/HTML Scalable object with an image.
 *
 * @since 2.1.0
 */
/*
 * TODO: All this animation code could be combined much better with
 *       the image code.  It couldn't be done when it was written
 *       because it requires breaking backward compatibility.  It
 *       would be good to do it for 3.0.0.
 */
GtkHtmlScalable *gtk_html_animation_new(GdkPixbufAnimation *img, const gchar *filename, int id);

/**
 * Destroys and frees a GTK+ IM/HTML scalable image.
 *
 * @param scale The GTK+ IM/HTML scalable.
 */
/* TODO: Is there any reason this isn't private? */
void gtk_html_image_free(GtkHtmlScalable *scale);

/**
 * Destroys and frees a GTK+ IM/HTML scalable animation.
 *
 * @param scale The GTK+ IM/HTML scalable.
 */
/* TODO: Is there any reason this isn't private? */
void gtk_html_animation_free(GtkHtmlScalable *scale);

/**
 * Rescales a GTK+ IM/HTML scalable image to a given size.
 *
 * @param scale  The GTK+ IM/HTML scalable.
 * @param width  The new width.
 * @param height The new height.
 */
/* TODO: Is there any reason this isn't private? */
void gtk_html_image_scale(GtkHtmlScalable *scale, int width, int height);

/**
 * Adds a GTK+ IM/HTML scalable image to a given GTK+ IM/HTML at a given iter.
 *
 * @param scale  The GTK+ IM/HTML scalable.
 * @param imhtml The GTK+ IM/HTML.
 * @param iter   The GtkTextIter at which to add the scalable.
 */
/* TODO: Is there any reason this isn't private? */
void gtk_html_image_add_to(GtkHtmlScalable *scale, GtkHtml *imhtml, GtkTextIter *iter);

/**
 * Creates and returns an new GTK+ IM/HTML scalable with a horizontal rule.
 *
 * @return A new IM/HTML Scalable object with an image.
 */
GtkHtmlScalable *gtk_html_hr_new(void);

/**
 * Destroys and frees a GTK+ IM/HTML scalable horizontal rule.
 *
 * @param scale The GTK+ IM/HTML scalable.
 */
void gtk_html_hr_free(GtkHtmlScalable *scale);

/**
 * Rescales a GTK+ IM/HTML scalable horizontal rule to a given size.
 *
 * @param scale  The GTK+ IM/HTML scalable.
 * @param width  The new width.
 * @param height The new height.
 */
void gtk_html_hr_scale(GtkHtmlScalable *scale, int width, int height);

/**
 * Adds a GTK+ IM/HTML scalable horizontal rule to a given GTK+ IM/HTML at
 * a given iter.
 *
 * @param scale  The GTK+ IM/HTML scalable.
 * @param imhtml The GTK+ IM/HTML.
 * @param iter   The GtkTextIter at which to add the scalable.
 */
void gtk_html_hr_add_to(GtkHtmlScalable *scale, GtkHtml *imhtml, GtkTextIter *iter);

/**
 * Finds and highlights a given string in a GTK+ IM/HTML.
 *
 * @param imhtml The GTK+ IM/HTML.
 * @param text   The string to search for.
 *
 * @return @c TRUE if a search was performed, or @c FALSE if not.
 */
gboolean gtk_html_search_find(GtkHtml *imhtml, const gchar *text);

/**
 * Clears the highlighting from a prior search in a GTK+ IM/HTML.
 *
 * @param imhtml The GTK+ IM/HTML.
 */
void gtk_html_search_clear(GtkHtml *imhtml);

/**
 * Enables or disables editing in a GTK+ IM/HTML.
 *
 * @param imhtml   The GTK+ IM/HTML.
 * @param editable @c TRUE to make the widget editable, or @c FALSE otherwise.
 */
void gtk_html_set_editable(GtkHtml *imhtml, gboolean editable);

/**
 * Enables or disables whole buffer formatting only (wbfo) in a GTK+ IM/HTML.
 * In this mode formatting options to the buffer take effect for the entire
 * buffer instead of specific text.
 *
 * @param imhtml The GTK+ IM/HTML.
 * @param wbfo   @c TRUE to enable the mode, or @c FALSE otherwise.
 */
void gtk_html_set_whole_buffer_formatting_only(GtkHtml *imhtml, gboolean wbfo);

/**
 * Indicates which formatting functions to enable and disable in a GTK+ IM/HTML.
 *
 * @param imhtml  The GTK+ IM/HTML.
 * @param buttons A GtkHtmlButtons bitmask indicating which functions to use.
 */
void gtk_html_set_format_functions(GtkHtml *imhtml, GtkHtmlButtons buttons);

/**
 * Returns which formatting functions are enabled in a GTK+ IM/HTML.
 *
 * @param imhtml The GTK+ IM/HTML.
 *
 * @return A GtkHtmlButtons bitmask indicating which functions to are enabled.
 */
GtkHtmlButtons gtk_html_get_format_functions(GtkHtml *imhtml);

/**
 * Sets each boolean to @c TRUE or @c FALSE to indicate if that formatting option
 * is enabled at the current position in a GTK+ IM/HTML.
 *
 * @param imhtml    The GTK+ IM/HTML.
 * @param bold      The boolean to set for bold or @c NULL.
 * @param italic    The boolean to set for italic or @c NULL.
 * @param underline The boolean to set for underline or @c NULL.
 */
void gtk_html_get_current_format(GtkHtml *imhtml, gboolean *bold, gboolean *italic, gboolean *underline);

/**
 * Returns a string containing the selected font face at the current position
 * in a GTK+ IM/HTML.
 *
 * @param imhtml The GTK+ IM/HTML.
 *
 * @return A string containg the font face or @c NULL if none is set.
 */
char *gtk_html_get_current_fontface(GtkHtml *imhtml);

/**
 * Returns a string containing the selected foreground color at the current
 * position in a GTK+ IM/HTML.
 *
 * @param imhtml The GTK+ IM/HTML.
 *
 * @return A string containg the foreground color or @c NULL if none is set.
 */
char *gtk_html_get_current_forecolor(GtkHtml *imhtml);

/**
 * Returns a string containing the selected font background color at the current
 * position in a GTK+ IM/HTML.
 *
 * @param imhtml The GTK+ IM/HTML.
 *
 * @return A string containg the font background color or @c NULL if none is set.
 */
char *gtk_html_get_current_backcolor(GtkHtml *imhtml);

/**
 * Returns a string containing the selected background color at the current
 * position in a GTK+ IM/HTML.
 *
 * @param imhtml The GTK+ IM/HTML.
 *
 * @return A string containg the background color or @c NULL if none is set.
 */
char *gtk_html_get_current_background(GtkHtml *imhtml);

/**
 * Returns a integer containing the selected HTML font size at the current
 * position in a GTK+ IM/HTML.
 *
 * @param imhtml The GTK+ IM/HTML.
 *
 * @return The HTML font size.
 */
gint gtk_html_get_current_fontsize(GtkHtml *imhtml);

/**
 * Checks whether a GTK+ IM/HTML is marked as editable.
 *
 * @param imhtml The GTK+ IM/HTML.
 *
 * @return @c TRUE if the IM/HTML is editable, or @c FALSE otherwise.
 */
gboolean gtk_html_get_editable(GtkHtml *imhtml);

/**
 * Clear all the formatting on a GTK+ IM/HTML.
 *
 * @param imhtml The GTK+ IM/HTML.
 */
void gtk_html_clear_formatting(GtkHtml *imhtml);

/**
 * Toggles bold at the cursor location or selection in a GTK+ IM/HTML.
 *
 * @param imhtml The GTK+ IM/HTML.
 */
void gtk_html_toggle_bold(GtkHtml *imhtml);

/**
 * Toggles italic at the cursor location or selection in a GTK+ IM/HTML.
 *
 * @param imhtml The GTK+ IM/HTML.
 */
void gtk_html_toggle_italic(GtkHtml *imhtml);

/**
 * Toggles underline at the cursor location or selection in a GTK+ IM/HTML.
 *
 * @param imhtml The GTK+ IM/HTML.
 */
void gtk_html_toggle_underline(GtkHtml *imhtml);

/**
 * Toggles strikethrough at the cursor location or selection in a GTK+ IM/HTML.
 *
 * @param imhtml The GTK+ IM/HTML.
 */
void gtk_html_toggle_strike(GtkHtml *imhtml);

/**
 * Toggles a foreground color at the current location or selection in a GTK
 * IM/HTML.
 *
 * @param imhtml The GTK+ IM/HTML.
 * @param color  The HTML-style color, or @c NULL or "" to clear the color.
 *
 * @return @c TRUE if a color was set, or @c FALSE if it was cleared.
 */
gboolean gtk_html_toggle_forecolor(GtkHtml *imhtml, const char *color);

/**
 * Toggles a background color at the current location or selection in a GTK
 * IM/HTML.
 *
 * @param imhtml The GTK+ IM/HTML.
 * @param color  The HTML-style color, or @c NULL or "" to clear the color.
 *
 * @return @c TRUE if a color was set, or @c FALSE if it was cleared.
 */
gboolean gtk_html_toggle_backcolor(GtkHtml *imhtml, const char *color);

/**
 * Toggles a background color at the current location or selection in a GTK
 * IM/HTML.
 *
 * @param imhtml The GTK+ IM/HTML.
 * @param color  The HTML-style color, or @c NULL or "" to clear the color.
 *
 * @return @c TRUE if a color was set, or @c FALSE if it was cleared.
 */
gboolean gtk_html_toggle_background(GtkHtml *imhtml, const char *color);

/**
 * Toggles a font face at the current location or selection in a GTK+ IM/HTML.
 *
 * @param imhtml The GTK+ IM/HTML.
 * @param face   The font face name, or @c NULL or "" to clear the font.
 *
 * @return @c TRUE if a font name was set, or @c FALSE if it was cleared.
 */
gboolean gtk_html_toggle_fontface(GtkHtml *imhtml, const char *face);

/**
 * Toggles a link tag with the given URL at the current location or selection
 * in a GTK+ IM/HTML.
 *
 * @param imhtml The GTK+ IM/HTML.
 * @param url    The URL for the link or @c NULL to terminate the link.
 */
void gtk_html_toggle_link(GtkHtml *imhtml, const char *url);

/**
 * Inserts a link to the given url at the given GtkTextMark in a GTK+ IM/HTML.
 *
 * @param imhtml The GTK+ IM/HTML.
 * @param mark   The GtkTextMark to insert the link at.
 * @param url    The URL for the link.
 * @param text   The string to use for the link description.
 */
void gtk_html_insert_link(GtkHtml *imhtml, GtkTextMark *mark, const char *url, const char *text);

/**
 * Inserts a smiley at the current location or selection in a GTK+ IM/HTML.
 *
 * @param imhtml The GTK+ IM/HTML.
 * @param sml    The category of the smiley.
 * @param smiley The text of the smiley to insert.
 */
void gtk_html_insert_smiley(GtkHtml *imhtml, const char *sml, char *smiley);
/**
 * Inserts a smiley at the given iter in a GTK+ IM/HTML.
 *
 * @param imhtml The GTK+ IM/HTML.
 * @param sml    The category of the smiley.
 * @param smiley The text of the smiley to insert.
 * @param iter   The GtkTextIter in the IM/HTML to insert the smiley at.
 */
void gtk_html_insert_smiley_at_iter(GtkHtml *imhtml, const char *sml, char *smiley, GtkTextIter *iter);

/**
 * Inserts the IM/HTML scalable image with the given id at the given iter in a
 * GTK+ IM/HTML.
 *
 * @param imhtml The GTK+ IM/HTML.
 * @param id     The id of the IM/HTML scalable.
 * @param iter   The GtkTextIter in the IM/HTML to insert the image at.
 */
void gtk_html_insert_image_at_iter(GtkHtml *imhtml, int id, GtkTextIter *iter);

/**
 * Sets the font size at the current location or selection in a GTK+ IM/HTML.
 *
 * @param imhtml The GTK+ IM/HTML.
 * @param size   The HTML font size to use.
 */
void gtk_html_font_set_size(GtkHtml *imhtml, gint size);

/**
 * Decreases the font size by 1 at the current location or selection in a GTK
 * IM/HTML.
 *
 * @param imhtml The GTK+ IM/HTML.
 */
void gtk_html_font_shrink(GtkHtml *imhtml);

/**
 * Increases the font size by 1 at the current location or selection in a GTK
 * IM/HTML.
 *
 * @param imhtml The GTK+ IM/HTML.
 */
void gtk_html_font_grow(GtkHtml *imhtml);

/**
 * Returns the HTML formatted contents between two iters in a GTK+ IM/HTML.
 *
 * @param imhtml The GTK+ IM/HTML.
 * @param start  The GtkTextIter indicating the start point in the IM/HTML.
 * @param end    The GtkTextIter indicating the end point in the IM/HTML.
 *
 * @return A string containing the HTML formatted text.
 */
char *gtk_html_get_markup_range(GtkHtml *imhtml, GtkTextIter *start, GtkTextIter *end);

/**
 * Returns the entire HTML formatted contents of a GTK+ IM/HTML.
 *
 * @param imhtml The GTK+ IM/HTML.
 *
 * @return A string containing the HTML formatted text.
 */
char *gtk_html_get_markup(GtkHtml *imhtml);

/**
 * Returns a null terminated array of pointers to null terminated strings, each
 * string for each line.  g_strfreev() should be called to free it when done.
 *
 * @param imhtml The GTK+ IM/HTML.
 *
 * @return A null terminated array of null terminated HTML formatted strings.
 */
char **gtk_html_get_markup_lines(GtkHtml *imhtml);

/**
 * Returns the entire unformatted (plain text) contents of a GTK+ IM/HTML
 * between two iters in a GTK+ IM/HTML.
 *
 * @param imhtml The GTK+ IM/HTML.
 * @param start  The GtkTextIter indicating the start point in the IM/HTML.
 * @param stop   The GtkTextIter indicating the end point in the IM/HTML.
 *
 * @return A string containing the unformatted text.
 */
char *gtk_html_get_text(GtkHtml *imhtml, GtkTextIter *start, GtkTextIter *stop);

/**
 * Setup formatting for an imhtml depending on the flags specified.
 *
 * @param imhtml  The GTK+ IM/HTML.
 * @param flags   The connection flag which describes the allowed types of formatting.
 *
 * @since 2.1.0
 */
void gtk_html_setup_entry(GtkHtml *html);

/*@}*/

#ifdef __cplusplus
}
#endif

#endif /* _BEASYIMHTML_H_ */

