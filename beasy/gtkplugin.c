...
#define BEASY_RESPONSE_CONFIGURE 98121

static void plugin_toggled_stage_two(OulPlugin *plug, GtkTreeModel *model,
...
GtkWidget *
beasy_plugin_get_config_frame(OulPlugin *plugin)
{
	GtkWidget *config = NULL;

	g_return_val_if_fail(plugin != NULL, NULL);

	if (BEASY_IS_PIDGIN_PLUGIN(plugin) && plugin->info->ui_info
		&& BEASY_PLUGIN_UI_INFO(plugin)->get_config_frame)
	{
		BeasyPluginUiInfo *ui_info;

		ui_info = BEASY_PLUGIN_UI_INFO(plugin);
		...
		&& plugin->info->prefs_info->get_plugin_pref_frame)
	{
		...
		config = beasy_plugin_pref_create_frame(frame);
...
void
beasy_plugins_save(void)
{
	oul_plugins_save_loaded(BEASY_PREFS_PLUGIN_LOADED);
}
...
update_plugin_list(void *data)
{
	...
	for (probes = oul_plugins_get_all();
		 probes != NULL;
		 probes = probes->next)
	{
		char *name;
		...
		if (plug->info->type == OUL_PLUGIN_LOADER) {
			GList *cur;
			for (cur = OUL_PLUGIN_LOADER_INFO(plug)->exts; cur != NULL;
					 cur = cur->next)
				oul_plugins_probe(cur->data);
			continue;
		} else if (plug->info->type != OUL_PLUGIN_STANDARD ||
			(plug->info->flags & OUL_PLUGIN_FLAG_INVISIBLE)) {
			continue;
		}
		...
		version = g_markup_escape_text(oul_plugin_get_version(plug), -1);
		summary = g_markup_escape_text(oul_plugin_get_summary(plug), -1);
		...
		gtk_list_store_set(ls, &iter,
				   0, oul_plugin_is_loaded(plug),
				   1, desc,
				   2, plug,
				   3, oul_plugin_is_unloadable(plug),
				   -1);
		g_free(desc);
	}
}

static void plugin_loading_common(OulPlugin *plugin, GtkTreeView *view, gboolean loaded)
{
	GtkTreeIter iter;
			...
			if (gtk_tree_selection_get_selected(sel, &model, &iter))
			{
				...
				if (plug == plugin)
				{
					gtk_widget_set_sensitive(pref_button,
						loaded
						&& ((BEASY_IS_PIDGIN_PLUGIN(plug) && plug->info->ui_info
							&& BEASY_PLUGIN_UI_INFO(plug)->get_config_frame)
...
static void plugin_load_cb(OulPlugin *plugin, gpointer data)
{
	GtkTreeView *view = (GtkTreeView *)data;
	plugin_loading_common(plugin, view, TRUE);
}

static void plugin_unload_cb(OulPlugin *plugin, gpointer data)
{
	GtkTreeView *view = (GtkTreeView *)data;
	plugin_loading_common(plugin, view, FALSE);
}

static void pref_dialog_response_cb(GtkWidget *d, int response, OulPlugin *plug)
{
	switch (response) {
	...
	case GTK_RESPONSE_DELETE_EVENT:
		...
		if (plug->info->prefs_info && plug->info->prefs_info->frame) {
			oul_plugin_pref_frame_destroy(plug->info->prefs_info->frame);
...
{
	OulPlugin *plugin = (OulPlugin *)data[0];
...
static void plugin_toggled(GtkCellRendererToggle *cell, gchar *pth, gpointer data)
{
	...
	if (oul_plugin_is_unloadable(plug))
	{
		g_free(iter);
		return;
	}

	if (!oul_plugin_is_loaded(plug))
	{
		beasy_set_cursor(plugin_dialog, GDK_WATCH);

		oul_plugin_load(plug);
		plugin_toggled_stage_two(plug, model, iter, FALSE);

		beasy_clear_cursor(plugin_dialog);
	}
	else
	{
		...
		if (plug->dependent_plugins != NULL)
		{
			...
			for (l = plug->dependent_plugins ; l != NULL ; l = l->next)
			{
				...
				OulPlugin *dep_plugin = oul_plugins_find_with_id(dep_name);
				g_return_if_fail(dep_plugin != NULL);

				g_string_append_printf(tmp, "\n\t%s\n", oul_plugin_get_name(dep_plugin));
			}
			...
			/**
			oul_request_action(plugin_dialog, NULL,
...
static void plugin_toggled_stage_two(OulPlugin *plug, GtkTreeModel *model, GtkTreeIter *iter, gboolean unload)
{
	if (unload)
	{
		beasy_set_cursor(plugin_dialog, GDK_WATCH);

		if (!oul_plugin_unload(plug))
		{
			...
			if (!plug->error)
			{
				oul_notify_warning(NULL, NULL, primary, reload);
			}
			else
			{
				char *tmp = g_strdup_printf("%s\n\n%s", reload, plug->error);
				oul_notify_warning(NULL, NULL, primary, tmp);
				g_free(tmp);
			}

			oul_plugin_disable(plug);
		}
	...
	if (plug->error != NULL)
	{
		gchar *name = g_markup_escape_text(oul_plugin_get_name(plug), -1);
		...
		text = g_strdup_printf(
			...
			oul_plugin_get_name(plug), oul_plugin_get_version(plug), error);
...
static void prefs_plugin_sel (GtkTreeSelection *sel, GtkTreeModel *model)
{
	...
	name = g_markup_escape_text(oul_plugin_get_name(plug), -1);
	version = g_markup_escape_text(oul_plugin_get_version(plug), -1);
	buf = g_strdup_printf(
	...
	gtk_text_buffer_set_text(plugin_desc, oul_plugin_get_description(plug), -1);
	gtk_label_set_text(plugin_author, oul_plugin_get_author(plug));
	gtk_label_set_text(plugin_filename, plug->path);

	g_free(plugin_website_uri);
	plugin_website_uri = g_strdup(oul_plugin_get_homepage(plug));
...
static void plugin_dialog_response_cb(GtkWidget *d, int response, GtkTreeSelection *sel)
{
	...
	case GTK_RESPONSE_CLOSE:
	case GTK_RESPONSE_DELETE_EVENT:
		//oul_request_close_with_handle(plugin_dialog);
		oul_signals_disconnect_by_handle(plugin_dialog);
		gtk_widget_destroy(d);
		if (plugin_pref_dialogs != NULL) {
			g_hash_table_destroy(plugin_pref_dialogs);
			plugin_pref_dialogs = NULL;
	...
	case BEASY_RESPONSE_CONFIGURE:
		...
		if (plugin_pref_dialogs != NULL && g_hash_table_lookup(plugin_pref_dialogs, plug))
			break;

		box = beasy_plugin_get_config_frame(plug);
		if (box == NULL) break;

		dialog = gtk_dialog_new_with_buttons(BEASY_ALERT_TITLE, GTK_WINDOW(d),
		...
		gtk_window_set_title(GTK_WINDOW(dialog), _(oul_plugin_get_name(plug)));
...
show_plugin_prefs_cb(GtkTreeView *view, GtkTreePath *path, GtkTreeViewColumn *column, GtkWidget *dialog)
{
	...
	plugin_dialog_response_cb(dialog, BEASY_RESPONSE_CONFIGURE, sel);
}

static gboolean
beasy_plugins_paint_tooltip(GtkWidget *tipwindow, gpointer data)
{
...
static gboolean
beasy_plugins_create_tooltip(GtkWidget *tipwindow, GtkTreePath *path,
		gpointer data, int *w, int *h)
{
	...
	markup = g_strdup_printf("<span size='x-large' weight='bold'>%s</span>\n<b>%s:</b> %s\n<b>%s:</b> %s",
			name = g_markup_escape_text(oul_plugin_get_name(plugin), -1),
			_("Description"), desc = g_markup_escape_text(oul_plugin_get_description(plugin), -1),
			_("Author"), author = g_markup_escape_text(oul_plugin_get_author(plugin), -1));
...
{
	if (plugin_website_uri) {
		beasy_set_cursor(button, GDK_HAND2);
		return TRUE;
	}
	return FALSE;
}
...
{
	if (plugin_website_uri) {
		oul_notify_uri(NULL, plugin_website_uri);
		return TRUE;
	}
	return FALSE;
}
...
create_details()
{
	...
	g_object_set(view, "wrap-mode", GTK_WRAP_WORD,
	                   "editable",  FALSE,
	                   "left-margin",  BEASY_HIG_CAT_SPACE,
	                   "right-margin", BEASY_HIG_CAT_SPACE,
	                   NULL);
	...
	beasy_add_widget_to_vbox(vbox, "", sg,
	...
	g_signal_connect(website_button, "leave-notify-event",
		G_CALLBACK(beasy_clear_cursor), NULL);

	beasy_add_widget_to_vbox(vbox, "", sg, website_button, TRUE, &label);
...
void beasy_plugin_dialog_show()
{
	...
	pref_button = gtk_dialog_add_button(GTK_DIALOG(plugin_dialog),
						_("Configure Pl_ugin"), BEASY_RESPONSE_CONFIGURE);
	...
	oul_signal_connect(oul_plugins_get_handle(), "plugin-load", plugin_dialog,
	                    OUL_CALLBACK(plugin_load_cb), event_view);
	oul_signal_connect(oul_plugins_get_handle(), "plugin-unload", plugin_dialog,
	                    OUL_CALLBACK(plugin_unload_cb), event_view);
	...
	gtk_tree_view_set_search_equal_func(GTK_TREE_VIEW(event_view),
				beasy_tree_view_search_equal_func, NULL, NULL);
