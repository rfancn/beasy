...
OulCircBuffer *
oul_circ_buffer_new(gsize growsize) {
	OulCircBuffer *buf = g_new0(OulCircBuffer, 1);
...
static void grow_circ_buffer(OulCircBuffer *buf, gsize len) {
...
gboolean oul_circ_buffer_mark_read(OulCircBuffer *buf, gsize len) {
	...
	g_return_val_if_fail(oul_circ_buffer_get_max_read(buf) >= len, FALSE);
