...
/* typedef struct _OulUPnPRequestData OulUPnPRequestData; */

typedef void (*OulUPnPCallback) (gboolean success, gpointer data);


/**
 * Initialize UPnP
 */
void oul_upnp_init(void);
 ...
 */
void oul_upnp_discover(OulUPnPCallback cb, gpointer cb_data);

#if 0
/**
 * Retrieve the current UPnP control info, if there is any available.
 * This will only be filled in if oul_upnp_discover() had been called,
 * and finished discovering.
 *
 * @return The control URL for the IGD we'll use to use the IGD services
 */
const OulUPnPControlInfo* oul_upnp_get_control_info(void);
#endif
 ...
 */
const gchar* oul_upnp_get_public_ip(void);

/**
 * Cancel a pending port mapping request initiated with either
 * oul_upnp_set_port_mapping() or oul_upnp_remove_port_mapping().
 *
 * @param mapping_data The data returned when you initiated the UPnP mapping request.
 */
void oul_upnp_cancel_port_mapping(UPnPMappingAddRemove *mapping_data);

/**
 * Maps Ports in a UPnP enabled IGD that sits on the local network to
 * this oul client. Essentially, this function takes care of the port
 ...
 * @param cb_data Extra data to be passed to the callback
 *
 * @return Data which can be passed to oul_upnp_port_mapping_cancel() to cancel
 */
UPnPMappingAddRemove *oul_upnp_set_port_mapping(unsigned short portmap, const gchar* protocol,
		OulUPnPCallback cb, gpointer cb_data);

/**
 * Deletes a port mapping in a UPnP enabled IGD that sits on the local network
 * to this oul client. Essentially, this function takes care of deleting the
 ...
 * @return Data which can be passed to oul_upnp_port_mapping_cancel() to cancel
 */
UPnPMappingAddRemove *oul_upnp_remove_port_mapping(unsigned short portmap,
		const gchar* protocol, OulUPnPCallback cb, gpointer cb_data);
