...
typedef struct _OulMenuAction
{
	char *label;
	OulCallback callback;
	gpointer data;
	GList *children;
} OulMenuAction;

typedef char *(*OulInfoFieldFormatCallback)(const char *field, size_t len);

/**
 * A key-value pair.
 *
 * This is used by, among other things, oul_gtk_combo* functions to pass in a
 * list of key-value pairs so it can display a user-friendly value.
 */
typedef struct _OulKeyValuePair
{
	gchar *key;
	void *value;
 ...
 * @since 2.4.0
 */
void oul_util_set_current_song(const char *title, const char *artist,
		const char *album);
 ...
 * @since 2.4.0
 */
char * oul_util_format_song_info(const char *title, const char *artist,
 ...
 *
 * @see oul_base16_decode()
 */
gchar *oul_base16_encode(const guchar *data, gsize len);
 ...
 *
 * @see oul_base16_encode()
 */
guchar *oul_base16_decode(const char *str, gsize *ret_len);
 ...
 *         g_free'd when no longer needed.
 */
gchar *oul_base16_encode_chunked(const guchar *data, gsize len);
 ...
 *         g_free'd when no longer needed.
 *
 * @see oul_base64_decode()
 */
gchar *oul_base64_encode(const guchar *data, gsize len);
 ...
 * @return The raw data.  Must be g_free'd when no longer needed.
 *
 * @see oul_base64_encode()
 */
guchar *oul_base64_decode(const char *str, gsize *ret_len);
 ...
 */
guchar *oul_quotedp_decode(const char *str, gsize *ret_len);
 ...
 *         needed.
 */
char *oul_mime_decode_field(const char *str);
 ...
 */
const char *oul_date_format_short(const struct tm *tm);
 ...
 */
const char *oul_date_format_long(const struct tm *tm);
 ...
 */
const char *oul_date_format_full(const struct tm *tm);
 ...
 */
const char *oul_time_format(const struct tm *tm);
 ...
 * @return A time_t.
 */
time_t oul_time_build(int year, int month, int day, int hour,
					   int min, int sec);

/** Used by oul_str_to_time to indicate no timezone offset was
  * specified in the timestamp string. */
#define OUL_NO_TZ_OFF -500000
 ...
 *                  the value of the macro @c OUL_NO_TZ_OFF indicates no
 ...
 * @return A time_t.
 */
time_t oul_str_to_time(const char *timestamp, gboolean utc,
 ...
 */
gboolean oul_markup_find_tag(const char *needle, const char *haystack,
 ...
 */
void oul_markup_html_to_xhtml(const char *html, char **dest_xhtml,
							   char **dest_plain);
 ...
 */
char *oul_markup_strip_html(const char *str);
 ...
 */
char *oul_markup_linkify(const char *str);
 ...
 */
char *oul_unescape_html(const char *html);
 ...
 * this function may be sensitive to changes in GtkHtml and may break
 * when used with other UI's. liboul users are encouraged to report and
 ...
 */
char *oul_markup_slice(const char *str, guint x, guint y);
 ...
 */
char *oul_markup_get_tag_name(const char *tag);

/**
 * Returns a constant string of the character representation of the HTML
 * entity pointed to by @a text. For example, oul_markup_unescape_entity("&amp;")
 ...
 * returns @c NULL.
 *
 * Note that this function, unlike oul_unescape_html(), does not search
 ...
 */
const char * oul_markup_unescape_entity(const char *text, int *length);
 ...
 *
 * For example, oul_markup_get_css_property("direction:rtl;color:#dc4d1b;",
 ...
 */
char * oul_markup_get_css_property(const gchar *style, const gchar *opt);
 ...
 * @see oul_user_dir()
 */
const gchar*    oul_home_dir(void);

/**
 * Returns the oul settings directory in the user's home directory.
 * This is usually ~/.oul
 *
 * @return The oul settings directory.
 *
 * @see oul_home_dir()
 */
const char*    oul_user_dir(void);

/**
 * Define a custom oul settings directory, overriding the default (user's home directory/.oul)
...
/**
 * Write a string of data to a file of the given name in the Oul
 * user directory ($HOME/.oul by default).  The data is typically
 * a serialized version of one of Oul's config files, such as
 ...
 *
 * @param filename The basename of the file to write in the oul_user_dir.
 ...
 */
gboolean oul_util_write_data_to_file(const char *filename, const char *data,
									  gssize size);
 ...
 *       rolls around.
 * @see oul_util_write_data_to_file()
 *
 */
gboolean
 ...
 * Oul's configuration xml files (prefs.xml, pounces.xml, etc.)
 *
 * @param filename    The basename of the file to open in the oul_user_dir.
 ...
 */
xmlnode *oul_util_read_xml_from_file(const char *filename,
 ...
 */
FILE *oul_mkstemp(char **path, gboolean binary);
 ...
 *         oul_util_get_image_extension().  This return value must
 *         be g_freed by the caller.
 */
char *oul_util_get_image_filename(gconstpointer image_data, size_t image_len);
 ...
 */
gboolean oul_program_is_valid(const char *program);
 ...
 */
gboolean oul_running_gnome(void);
 ...
 */
gboolean oul_str_has_prefix(const char *s, const char *p);
 ...
 */
gboolean oul_str_has_suffix(const char *s, const char *x);
 ...
 */
gchar *oul_strdup_withhtml(const gchar *src);
 ...
 */
char *oul_str_add_cr(const char *str);
 ...
 */
void oul_util_chrreplace(char *string, char delimiter,
						  char replacement);
 ...
 */
gchar *oul_strreplace(const char *string, const char *delimiter,
...
*/
char *oul_utf8_ncr_encode(const char *in);
 ...
 */
char *oul_utf8_ncr_decode(const char *in);
 ...
 *         free this with g_free().
 */
gchar *oul_strcasereplace(const char *string, const char *delimiter,
						   const char *replacement);
 ...
 */
const char *oul_strcasestr(const char *haystack, const char *needle);
 ...
 */
char *oul_str_size_to_units(size_t size);
 ...
 *         seconds.
 */
char *oul_str_seconds_to_string(guint sec);
 ...
 */
char *oul_str_binary_to_ascii(const unsigned char *binary, guint len);
/*@}*/
...
void oul_got_protocol_handler_uri(const char *uri);
 ...
 */
gboolean oul_email_is_valid(const char *address);
 ...
 */
gboolean oul_ip_address_is_valid(const char *ip);
 ...
 */
GList *oul_uri_list_extract_uris(const gchar *uri_list);
 ...
 *         oul_uri_list_extract_uris() function, this will discard
 *         any non-file uri from the result value.
 */
GList *oul_uri_list_extract_filenames(const gchar *uri_list);
 ...
 */
gchar *oul_utf8_try_convert(const char *str);
 ...
 */
gchar *oul_utf8_salvage(const char *str);
 ...
 * @since 2.4.0
 */
G_CONST_RETURN gchar *oul_gai_strerror(gint errnum);
 ...
 */
int oul_utf8_strcasecmp(const char *a, const char *b);
 ...
 */
gboolean oul_utf8_has_word(const char *haystack, const char *needle);
 ...
 */
void oul_print_utf8_to_console(FILE *filestream, char *message);
 ...
 * @return x + 8
 */
#define oul_add_eight(x) ((x)+8)

/**
 * Does the reverse of oul_escape_filename
 *
 ...
 */
const char *oul_unescape_filename(const char *str);
 ...
 * @return The resulting string.
 */
const char *oul_escape_filename(const char *str);
