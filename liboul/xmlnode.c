...
new_node(const char *name, XMLNodeType type)
{
	...
	//OUL_DBUS_REGISTER_POINTER(node, xmlnode);
...
xmlnode_free(xmlnode *node)
{
	...
	//OUL_DBUS_UNREGISTER_POINTER(node);
	g_free(node);
}
...
xmlnode_get_data_unescaped(xmlnode *node)
{
	...
	char *unescaped = escaped ? oul_unescape_html(escaped) : NULL;
...
xmlnode_parser_error_libxml(void *user_data, const char *msg, ...)
{
	...
	oul_debug_error("xmlnode", "Error parsing xml file: %s\n", errmsg);
}
