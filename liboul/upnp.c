...
/**
 * Universal Plug and Play is a set of computer network protocols 
...
#define PORT_MAPPING_DESCRIPTION "OUL_UPNP_PORT_FORWARD"
...
typedef enum {
	OUL_UPNP_STATUS_UNDISCOVERED = -1,
	OUL_UPNP_STATUS_UNABLE_TO_DISCOVER,
	OUL_UPNP_STATUS_DISCOVERING,
	OUL_UPNP_STATUS_DISCOVERED
} OulUPnPStatus;
...
typedef struct {
	guint inpa;	/* oul_input_add handle */
	guint tima;	/* oul_timeout_add handle */
	int fd;
...
static OulUPnPControlInfo control_info = {
	OUL_UPNP_STATUS_UNDISCOVERED,
	NULL, "\0", "\0", "\0", 0};

static GSList *discovery_callbacks = NULL;

static void oul_upnp_discover_send_broadcast(UPnPDiscoveryData *dd);
...
fire_discovery_callbacks(gboolean success)
{
	while(discovery_callbacks) {
		gpointer data;
		OulUPnPCallback cb = discovery_callbacks->data;
...
static gboolean
oul_upnp_compare_device(const xmlnode* device, const gchar* deviceType)
{
...
static gboolean
oul_upnp_compare_service(const xmlnode* service, const gchar* serviceType)
{
...
static gchar*
oul_upnp_parse_description_response(const gchar* httpResponse, gsize len,
	const gchar* httpURL, const gchar* serviceType)
{
	...
	while(!oul_upnp_compare_device(serviceTypeNode,
	...
	serviceTypeNode = xmlnode_get_child(serviceTypeNode, "device");
	while(serviceTypeNode && !oul_upnp_compare_device(serviceTypeNode,
	...
	while(!oul_upnp_compare_service(serviceTypeNode, service) &&
			serviceTypeNode != NULL) {
		serviceTypeNode = xmlnode_get_next_twin(serviceTypeNode);
	}
	...
	if(baseURL && !oul_str_has_prefix(tmp, "http://") &&
	   !oul_str_has_prefix(tmp, "HTTP://")) {
...
static void
upnp_parse_description_cb(OulHttpFetchUrlData *url_data, gpointer user_data,
		const gchar *httpResponse, gsize len, const gchar *error_message)
{
	...
	if (len > 0)
		control_url = oul_upnp_parse_description_response(
	...
	if(control_url == NULL) {
		oul_debug_error("upnp",
			"oul_upnp_parse_description(): control URL is NULL\n");
	}

	control_info.status = control_url ? OUL_UPNP_STATUS_DISCOVERED
		: OUL_UPNP_STATUS_UNABLE_TO_DISCOVER;
...
static void
oul_upnp_parse_description(const gchar* descriptionURL, UPnPDiscoveryData *dd)
{
	gchar* httpRequest;
	...
	if(!oul_url_parse(descriptionURL, &descriptionAddress,
	...
	oul_timeout_remove(dd->tima);
	dd->tima = 0;

	oul_http_fetch_url_request_len(descriptionURL, TRUE, NULL, TRUE, httpRequest,
...
static void
oul_upnp_parse_discover_response(const gchar* buf, unsigned int buf_len,
	UPnPDiscoveryData *dd)
{
	...
	oul_upnp_parse_description(descURL, dd);
...
static gboolean
oul_upnp_discover_timeout(gpointer data)
{
	UPnPDiscoveryData* dd = data;

	if (dd->inpa)
		oul_input_remove(dd->inpa);
	dd->inpa = 0;
	dd->tima = 0;
	...
	if (dd->retry_count < NUM_UDP_ATTEMPTS) {
		...
		dd->retry_count++;
		oul_upnp_discover_send_broadcast(dd);
	} else {
		if (dd->fd)
			close(dd->fd);

		control_info.status = OUL_UPNP_STATUS_UNABLE_TO_DISCOVER;
		control_info.lookup_time = time(NULL);
...
static void
oul_upnp_discover_udp_read(gpointer data, gint sock, OulInputCondition cond)
{
	int len;
	UPnPDiscoveryData *dd = data;
	gchar buf[65536];
	...
	oul_upnp_parse_discover_response(buf, len, dd);

	/* We'll either time out or continue successfully */
}

static void
oul_upnp_discover_send_broadcast(UPnPDiscoveryData *dd)
{
	...
	for(; dd->retry_count < NUM_UDP_ATTEMPTS; dd->retry_count++) {
		...
		if(sentSuccess) {
			dd->tima = oul_timeout_add(DISCOVERY_TIMEOUT,
				oul_upnp_discover_timeout, dd);
			dd->inpa = oul_input_add(dd->fd, OUL_INPUT_READ,
				oul_upnp_discover_udp_read, dd);
	...
	oul_timeout_add(10, oul_upnp_discover_timeout, dd);
}

void
oul_upnp_discover(OulUPnPCallback cb, gpointer cb_data)
{
	...
	if (control_info.status == OUL_UPNP_STATUS_DISCOVERING) {
		if (cb) {
	...
	if(sock == -1) {
		oul_debug_error("upnp",
			"oul_upnp_discover(): Failed In sock creation\n");
	...
	if((hp = gethostbyname(HTTPMU_HOST_ADDRESS)) == NULL) {
		oul_debug_error("upnp",
			"oul_upnp_discover(): Failed In gethostbyname\n");
		/* Short circuit the retry attempts */
		dd->retry_count = NUM_UDP_ATTEMPTS;
		oul_timeout_add(10, oul_upnp_discover_timeout, dd);
		return;
	...
	control_info.status = OUL_UPNP_STATUS_DISCOVERING;

	oul_upnp_discover_send_broadcast(dd);
}

static OulHttpFetchUrlData*
oul_upnp_generate_action_message_and_send(const gchar* actionName,
		const gchar* actionParams, OulHttpFetchUrlCallback cb,
		gpointer cb_data)
{
	OulHttpFetchUrlData* gfud;
	gchar* soapMessage;
	...
	/* parse the url into address, port, path variables */
	if(!oul_url_parse(control_info.control_url, &addressOfControl,
	...
	gfud = oul_http_fetch_url_request_len(control_info.control_url, FALSE, NULL, TRUE,
...
const gchar *
oul_upnp_get_public_ip()
{
	if (control_info.status == OUL_UPNP_STATUS_DISCOVERED
...
static void
looked_up_public_ip_cb(OulHttpFetchUrlData *url_data, gpointer user_data,
		const gchar *httpResponse, gsize len, const gchar *error_message)
{
	gchar* temp, *temp2;
	...
	*temp2 = '\0';
	...
	oul_debug_info("upnp", "NAT Returned IP: %s\n", control_info.publicip);
}

static void
lookup_public_ip()
{
	oul_upnp_generate_action_message_and_send("GetExternalIPAddress", "",
			looked_up_public_ip_cb, NULL);
}

/* TODO: This could be exported */
static const gchar *
oul_upnp_get_internal_ip(void)
{
	if (control_info.status == OUL_UPNP_STATUS_DISCOVERED
...
looked_up_internal_ip_cb(gpointer data, gint source, const gchar *error_message)
{
	if (source) {
		strncpy(control_info.internalip,
			oul_network_get_local_system_ip(source),
			sizeof(control_info.internalip));
		oul_debug_info("upnp", "Local IP: %s\n",
				control_info.internalip);
		close(source);
	} else
		oul_debug_info("upnp", "Unable to look up local IP\n");
...
lookup_internal_ip()
{
	...
	if(!oul_url_parse(control_info.control_url, &addressOfControl, &port,
			NULL, NULL, NULL)) {
		oul_debug_error("upnp",
	...
	if(oul_proxy_connect(NULL, addressOfControl, port, 
						looked_up_internal_ip_cb, NULL) == NULL)
	{
		oul_debug_error("upnp", "Get Local IP Connect Failed: Address: %s @@@ Port %d\n",
...
static void
done_port_mapping_cb(OulHttpFetchUrlData *url_data, gpointer user_data,
		const gchar *httpResponse, gsize len, const gchar *error_message)
{
	...
	{
		oul_debug_error("upnp",
			"oul_upnp_set_port_mapping(): Failed HTTP_OK\n%s\n",
			httpResponse ? httpResponse : "(null)");
		success =  FALSE;
	} else
		oul_debug_info("upnp", "Successfully completed port mapping operation\n");
...
do_port_mapping_cb(gboolean has_control_mapping, gpointer data)
{
	...
	if (has_control_mapping) {
		...
		if(ar->add) {
			...
			/* get the internal IP */
			if(!(internal_ip = oul_upnp_get_internal_ip())) {
				oul_debug_error("upnp",
					"oul_upnp_set_port_mapping(): couldn't get local ip\n");
				/* UGLY */
				if (ar->cb)
		...
		ar->gfud = oul_upnp_generate_action_message_and_send(action_name,
...
void oul_upnp_cancel_port_mapping(UPnPMappingAddRemove *ar)
{
	GSList *l;
	...
	if (ar->tima > 0)
		oul_timeout_remove(ar->tima);

	if (ar->gfud)
		oul_http_fetch_url_cancel(ar->gfud);

	g_free(ar);
}

UPnPMappingAddRemove *
oul_upnp_set_port_mapping(unsigned short portmap, const gchar* protocol,
		OulUPnPCallback cb, gpointer cb_data)
{
	...
	if(control_info.status == OUL_UPNP_STATUS_UNDISCOVERED ||
			(control_info.status == OUL_UPNP_STATUS_UNABLE_TO_DISCOVER
			 && (time(NULL) - control_info.lookup_time) > 300)) {
		oul_upnp_discover(do_port_mapping_cb, ar);
		return ar;
	} else if(control_info.status == OUL_UPNP_STATUS_UNABLE_TO_DISCOVER) {
		if (cb) {
			/* Asynchronously trigger a failed response */
			ar->tima = oul_timeout_add(10, fire_port_mapping_failure_cb, ar);
		} else {
...
UPnPMappingAddRemove *
oul_upnp_remove_port_mapping(unsigned short portmap, const char* protocol,
		OulUPnPCallback cb, gpointer cb_data)
{
	UPnPMappingAddRemove *ar;
...
static void
oul_upnp_network_config_changed_cb(void *data)
{
	/* Reset the control_info to default values */
	control_info.status = OUL_UPNP_STATUS_UNDISCOVERED;
	g_free(control_info.control_url);
	control_info.control_url = NULL;
	control_info.service_type[0] = '\0';
...
oul_upnp_init()
{
	oul_signal_connect(oul_network_get_handle(), "network-configuration-changed",
						  oul_upnp_get_handle(), OUL_CALLBACK(oul_upnp_network_config_changed_cb),
						  NULL);
}
