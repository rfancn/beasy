...
static void
md5_init(OulCipherContext *context, gpointer extra) {
	struct MD5Context *md5_context;

	md5_context = g_new0(struct MD5Context, 1);

	oul_cipher_context_set_data(context, md5_context);

	oul_cipher_context_reset(context, extra);
}

static void
md5_reset(OulCipherContext *context, gpointer extra) {
	struct MD5Context *md5_context;

	md5_context = oul_cipher_context_get_data(context);
...
static void
md5_uninit(OulCipherContext *context) {
	struct MD5Context *md5_context;

	oul_cipher_context_reset(context, NULL);
...
static void
md5_append(OulCipherContext *context, const guchar *data, size_t len) {
...
static gboolean
md5_digest(OulCipherContext *context, size_t in_len, guchar digest[16],
		   size_t *out_len)
{
	struct MD5Context *md5_context = NULL;
	guint32 last, pad;
...
static size_t
md5_get_block_size(OulCipherContext *context)
{
	/* This does not change (in this case) */
	return MD5_HMAC_BLOCK_SIZE;
}

static OulCipherOps MD5Ops = {
	NULL,			/* Set option */
	NULL,			/* Get option */
	md5_init,		/* init */
	md5_reset,		/* reset */
...
static void
md4_init(OulCipherContext *context, gpointer extra) {
	struct MD4_Context *mctx;
	mctx = g_new0(struct MD4_Context, 1);
	oul_cipher_context_set_data(context, mctx);
	oul_cipher_context_reset(context, extra);
...
static void
md4_reset(OulCipherContext *context, gpointer extra) {
	struct MD4_Context *mctx;

	mctx = oul_cipher_context_get_data(context);
...
static void
md4_append(OulCipherContext *context, const guchar *data, size_t len) 
{
	struct MD4_Context *mctx = oul_cipher_context_get_data(context);
...
static gboolean
md4_digest(OulCipherContext *context, size_t in_len, guchar *out,
		                   size_t *out_len)
{
	struct MD4_Context *mctx = oul_cipher_context_get_data(context);
...
static void
md4_uninit(OulCipherContext *context) {
	struct MD4_Context *md4_context;

	oul_cipher_context_reset(context, NULL);

	md4_context = oul_cipher_context_get_data(context);
...
static size_t
md4_get_block_size(OulCipherContext *context)
{
	/* This does not change (in this case) */
	return MD4_HMAC_BLOCK_SIZE;
}

static OulCipherOps MD4Ops = {
	NULL,                   /* Set option */
	NULL,                   /* Get option */
	md4_init,               /* init */
	md4_reset,              /* reset */
...
static void
hmac_init(OulCipherContext *context, gpointer extra)
{
	struct HMAC_Context *hctx;
	hctx = g_new0(struct HMAC_Context, 1);
	oul_cipher_context_set_data(context, hctx);
	oul_cipher_context_reset(context, extra);
}

static void
hmac_reset(OulCipherContext *context, gpointer extra)
{
	struct HMAC_Context *hctx;

	hctx = oul_cipher_context_get_data(context);

	g_free(hctx->name);
	hctx->name = NULL;
	if (hctx->hash)
		oul_cipher_context_destroy(hctx->hash);
	hctx->hash = NULL;
	hctx->blocksize = 0;
	g_free(hctx->opad);
	hctx->opad = NULL;
...
static void
hmac_set_opt(OulCipherContext *context, const gchar *name, void *value)
{
	struct HMAC_Context *hctx;
	...
	if (!strcmp(name, "hash")) {
		...
		hctx->hash = oul_cipher_context_new_by_name((char *)value, NULL);
		hctx->blocksize = oul_cipher_context_get_block_size(hctx->hash);
	}
}

static void *
hmac_get_opt(OulCipherContext *context, const gchar *name)
{
	struct HMAC_Context *hctx;
...
static void
hmac_append(OulCipherContext *context, const guchar *data, size_t len) 
{
	struct HMAC_Context *hctx = oul_cipher_context_get_data(context);

	g_return_if_fail(hctx->hash != NULL);

	oul_cipher_context_append(hctx->hash, data, len);
}

static gboolean
hmac_digest(OulCipherContext *context, size_t in_len, guchar *out, size_t *out_len)
{
	struct HMAC_Context *hctx = oul_cipher_context_get_data(context);
	OulCipherContext *hash = hctx->hash;
	guchar *inner_hash;
	size_t hash_len;
	gboolean result;
	...
	result = oul_cipher_context_digest(hash, 100, inner_hash, &hash_len);

	oul_cipher_context_reset(hash, NULL);

	oul_cipher_context_append(hash, hctx->opad, hctx->blocksize);
	oul_cipher_context_append(hash, inner_hash, hash_len);

	g_free(inner_hash);

	result = result && oul_cipher_context_digest(hash, in_len, out, out_len);

	return result;
}

static void
hmac_uninit(OulCipherContext *context)
{
	struct HMAC_Context *hctx;
...
static void
hmac_set_key_with_len(OulCipherContext *context, const guchar * key, size_t key_len)
{
	struct HMAC_Context *hctx = oul_cipher_context_get_data(context);
	int blocksize, i;
	guchar *ipad;
	...
	if (key_len > blocksize) {
		oul_cipher_context_reset(hctx->hash, NULL);
		oul_cipher_context_append(hctx->hash, key, key_len);
		full_key = g_malloc(100); /* TODO: Should be enough for now... */
		oul_cipher_context_digest(hctx->hash, 100, full_key, &key_len);
	} else
	...
	oul_cipher_context_reset(hctx->hash, NULL);
	oul_cipher_context_append(hctx->hash, ipad, blocksize);
	g_free(ipad);
}

static void
hmac_set_key(OulCipherContext *context, const guchar * key)
{
	hmac_set_key_with_len(context, key, strlen((char *)key));
}

static size_t 
hmac_get_block_size(OulCipherContext *context)
{
	struct HMAC_Context *hctx = oul_cipher_context_get_data(context);
 ...
 **/
static void
des_set_key (OulCipherContext *context, const guchar * key)
{
	struct _des_ctx *ctx = oul_cipher_context_get_data(context);
	int i;
...
static gint
des_encrypt(OulCipherContext *context, const guchar data[],
	...
	while(offset+8<=len) {
		des_ecb_crypt(oul_cipher_context_get_data(context),
				data+offset,
				output+offset,
				0);
		offset+=8;
...
static gint
des_decrypt(OulCipherContext *context, const guchar data[],
	    size_t len, guchar output[], size_t *outlen) {
	int offset = 0;
	int i = 0;
	int tmp;
...
static void
des_init(OulCipherContext *context, gpointer extra) {
...
static void
des_uninit(OulCipherContext *context) {
	struct _des_ctx *des_context;

	des_context = oul_cipher_context_get_data(context);
 ...
 **/
static void
des3_set_key(OulCipherContext *context, const guchar * key)
{
	struct _des3_ctx *ctx = oul_cipher_context_get_data(context);
	int i;
...
static gint
des3_encrypt(OulCipherContext *context, const guchar data[],
             size_t len, guchar output[], size_t *outlen)
{
	struct _des3_ctx *ctx = oul_cipher_context_get_data(context);

	if (ctx->mode == OUL_CIPHER_BATCH_MODE_ECB) {
		return des3_ecb_encrypt(ctx, data, len, output, outlen);
	} else if (ctx->mode == OUL_CIPHER_BATCH_MODE_CBC) {
...
static gint
des3_decrypt(OulCipherContext *context, const guchar data[],
             size_t len, guchar output[], size_t *outlen)
{
	struct _des3_ctx *ctx = oul_cipher_context_get_data(context);
...
static void
des3_set_batch(OulCipherContext *context, OulCipherBatchMode mode)
{
	struct _des3_ctx *ctx = oul_cipher_context_get_data(context);
...
static OulCipherBatchMode
des3_get_batch(OulCipherContext *context)
{
	struct _des3_ctx *ctx = oul_cipher_context_get_data(context);
...
static void
des3_set_iv(OulCipherContext *context, guchar *iv, size_t len)
{
	struct _des3_ctx *ctx;

	g_return_if_fail(len == 8);

	ctx = oul_cipher_context_get_data(context);

	memcpy(ctx->iv, iv, len);
}

static void
des3_init(OulCipherContext *context, gpointer extra)
{
...
static void
des3_uninit(OulCipherContext *context)
{
	struct _des3_ctx *des3_context;

	des3_context = oul_cipher_context_get_data(context);
...
static void
sha1_set_opt(OulCipherContext *context, const gchar *name, void *value) {
...
static void *
sha1_get_opt(OulCipherContext *context, const gchar *name) {
	struct SHA1Context *ctx;
...
static void
sha1_init(OulCipherContext *context, void *extra) {
	struct SHA1Context *sha1_ctx;

	sha1_ctx = g_new0(struct SHA1Context, 1);

	oul_cipher_context_set_data(context, sha1_ctx);

	oul_cipher_context_reset(context, extra);
}

static void
sha1_reset(OulCipherContext *context, void *extra) {
	struct SHA1Context *sha1_ctx;
	gint i;

	sha1_ctx = oul_cipher_context_get_data(context);
...
static void
sha1_uninit(OulCipherContext *context) {
	struct SHA1Context *sha1_ctx;
...
static void
sha1_append(OulCipherContext *context, const guchar *data, size_t len) {
	struct SHA1Context *sha1_ctx;
	gint i;
...
static gboolean
sha1_digest(OulCipherContext *context, size_t in_len, guchar digest[20],
			size_t *out_len)
{
	struct SHA1Context *sha1_ctx;
	...
	oul_cipher_context_append(context, &pad0x80, 1);
	while(sha1_ctx->lenW != 56)
		oul_cipher_context_append(context, &pad0x00, 1);
	oul_cipher_context_append(context, padlen, 8);
...
static size_t
sha1_get_block_size(OulCipherContext *context)
{
	/* This does not change (in this case) */
	return SHA1_HMAC_BLOCK_SIZE;
}

static OulCipherOps SHA1Ops = {
...
static void
rc4_init(OulCipherContext *context, void *extra) {
	struct RC4Context *rc4_ctx;
	rc4_ctx = g_new0(struct RC4Context, 1);
	oul_cipher_context_set_data(context, rc4_ctx);
	oul_cipher_context_reset(context, extra);
}


static void
rc4_reset(OulCipherContext *context, void *extra) {
	struct RC4Context *rc4_ctx;
	guint i;

	rc4_ctx = oul_cipher_context_get_data(context);
...
static void
rc4_uninit(OulCipherContext *context) {
	struct RC4Context *rc4_ctx;
...
static void
rc4_set_key (OulCipherContext *context, const guchar * key) {
...
static void
rc4_set_opt(OulCipherContext *context, const gchar *name, void *value) {
	struct RC4Context *ctx;
...
static size_t 
rc4_get_key_size (OulCipherContext *context)
{
	struct RC4Context *ctx;
...
static void *
rc4_get_opt(OulCipherContext *context, const gchar *name) {
	struct RC4Context *ctx;
...
static gint
rc4_encrypt(OulCipherContext *context, const guchar data[],
	    size_t len, guchar output[], size_t *outlen) {
	struct RC4Context *ctx;
	guchar temp_swap;
	guchar x, y, z;
...
struct _OulCipher {
	...
	OulCipherOps *ops; /**< Operations supported by this cipher */
	guint ref;            /**< Reference count */
};

struct _OulCipherContext {
	OulCipher *cipher; /**< Cipher this context is under */
 ...
 *****************************************************************************/
const gchar *
oul_cipher_get_name(OulCipher *cipher) {
...
guint
oul_cipher_get_capabilities(OulCipher *cipher) {
	OulCipherOps *ops = NULL;
	guint caps = 0;
	...
	if(ops->set_option)
		caps |= OUL_CIPHER_CAPS_SET_OPT;
	if(ops->get_option)
		caps |= OUL_CIPHER_CAPS_GET_OPT;
	if(ops->init)
		caps |= OUL_CIPHER_CAPS_INIT;
	if(ops->reset)
		caps |= OUL_CIPHER_CAPS_RESET;
	if(ops->uninit)
		caps |= OUL_CIPHER_CAPS_UNINIT;
	if(ops->set_iv)
		caps |= OUL_CIPHER_CAPS_SET_IV;
	if(ops->append)
		caps |= OUL_CIPHER_CAPS_APPEND;
	if(ops->digest)
		caps |= OUL_CIPHER_CAPS_DIGEST;
	if(ops->encrypt)
		caps |= OUL_CIPHER_CAPS_ENCRYPT;
	if(ops->decrypt)
		caps |= OUL_CIPHER_CAPS_DECRYPT;
	if(ops->set_salt)
		caps |= OUL_CIPHER_CAPS_SET_SALT;
	if(ops->get_salt_size)
		caps |= OUL_CIPHER_CAPS_GET_SALT_SIZE;
	if(ops->set_key)
		caps |= OUL_CIPHER_CAPS_SET_KEY;
	if(ops->get_key_size)
		caps |= OUL_CIPHER_CAPS_GET_KEY_SIZE;
	if(ops->set_batch_mode)
		caps |= OUL_CIPHER_CAPS_SET_BATCH_MODE;
	if(ops->get_batch_mode)
		caps |= OUL_CIPHER_CAPS_GET_BATCH_MODE;
	if(ops->get_block_size)
		caps |= OUL_CIPHER_CAPS_GET_BLOCK_SIZE;
	if(ops->set_key_with_len)
		caps |= OUL_CIPHER_CAPS_SET_KEY_WITH_LEN;

	return caps;
}

gboolean
oul_cipher_digest_region(const gchar *name, const guchar *data,
						  size_t data_len, size_t in_len,
						  guchar digest[], size_t *out_len)
{
	OulCipher *cipher;
	OulCipherContext *context;
	gboolean ret = FALSE;

	g_return_val_if_fail(name, FALSE);
	g_return_val_if_fail(data, FALSE);

	cipher = oul_ciphers_find_cipher(name);

	g_return_val_if_fail(cipher, FALSE);

	if(!cipher->ops->append || !cipher->ops->digest) {
		oul_debug_info("cipher", "oul_cipher_region failed: "
						"the %s cipher does not support appending and or "
						"digesting.", cipher->name);
		return FALSE;
	}

	context = oul_cipher_context_new(cipher, NULL);
	oul_cipher_context_append(context, data, data_len);
	ret = oul_cipher_context_digest(context, in_len, digest, out_len);
	oul_cipher_context_destroy(context);
 ...
 *****************************************************************************/
OulCipher *
oul_ciphers_find_cipher(const gchar *name) {
	OulCipher *cipher;
	GList *l;
...
OulCipher *
oul_ciphers_register_cipher(const gchar *name, OulCipherOps *ops) {
	OulCipher *cipher = NULL;

	g_return_val_if_fail(name, NULL);
	g_return_val_if_fail(ops, NULL);
	g_return_val_if_fail(!oul_ciphers_find_cipher(name), NULL);

	cipher = g_new0(OulCipher, 1);
	//OUL_DBUS_REGISTER_POINTER(cipher, OulCipher);
	...
	oul_signal_emit(oul_ciphers_get_handle(), "cipher-added", cipher);

	return cipher;
}

gboolean
oul_ciphers_unregister_cipher(OulCipher *cipher) {
	g_return_val_if_fail(cipher, FALSE);
	g_return_val_if_fail(cipher->ref == 0, FALSE);

	oul_signal_emit(oul_ciphers_get_handle(), "cipher-removed", cipher);

	ciphers = g_list_remove(ciphers, cipher);

	g_free(cipher->name);

	//OUL_DBUS_UNREGISTER_POINTER(cipher);
	g_free(cipher);
...
GList *
oul_ciphers_get_ciphers() {
	return ciphers;
}

/******************************************************************************
 * OulCipher Subsystem API
 *****************************************************************************/
gpointer
oul_ciphers_get_handle() {
	static gint handle;
...
oul_ciphers_init() {
	...
	handle = oul_ciphers_get_handle();

	oul_signal_register(handle, "cipher-added",
						 oul_marshal_VOID__POINTER, NULL, 1,
						 oul_value_new(OUL_TYPE_SUBTYPE,
										OUL_SUBTYPE_CIPHER));
	oul_signal_register(handle, "cipher-removed",
						 oul_marshal_VOID__POINTER, NULL, 1,
						 oul_value_new(OUL_TYPE_SUBTYPE,
										OUL_SUBTYPE_CIPHER));

	oul_ciphers_register_cipher("md5", &MD5Ops);
	oul_ciphers_register_cipher("sha1", &SHA1Ops);
	oul_ciphers_register_cipher("md4", &MD4Ops);
	oul_ciphers_register_cipher("hmac", &HMACOps);
	oul_ciphers_register_cipher("des", &DESOps);
	oul_ciphers_register_cipher("des3", &DES3Ops);
	oul_ciphers_register_cipher("rc4", &RC4Ops);
}
...
oul_ciphers_uninit() {
	...
	for(l = ciphers; l; l = ll) {
		...
		cipher = OUL_CIPHER(l->data);
		oul_ciphers_unregister_cipher(cipher);
	...
	oul_signals_unregister_by_instance(oul_ciphers_get_handle());
}
/******************************************************************************
 * OulCipherContext API
 *****************************************************************************/
void
oul_cipher_context_set_option(OulCipherContext *context, const gchar *name,
							   gpointer value)
{
	OulCipher *cipher = NULL;
	...
	if(cipher->ops && cipher->ops->set_option)
		...
		oul_debug_info("cipher", "the %s cipher does not support the "
						"set_option operation\n", cipher->name);
}

gpointer
oul_cipher_context_get_option(OulCipherContext *context, const gchar *name) {
	OulCipher *cipher = NULL;
...
OulCipherContext *
oul_cipher_context_new(OulCipher *cipher, void *extra) {
	OulCipherContext *context = NULL;

	g_return_val_if_fail(cipher, NULL);

	cipher->ref++;

	context = g_new0(OulCipherContext, 1);
...
OulCipherContext *
oul_cipher_context_new_by_name(const gchar *name, void *extra) {
	OulCipher *cipher;
	...
	return oul_cipher_context_new(cipher, extra);
}

void
oul_cipher_context_reset(OulCipherContext *context, void *extra) {
	OulCipher *cipher = NULL;
...
void
oul_cipher_context_destroy(OulCipherContext *context) {
	OulCipher *cipher = NULL;
...
void
oul_cipher_context_set_iv(OulCipherContext *context, guchar *iv, size_t len)
{
	OulCipher *cipher = NULL;
	...
	if(cipher->ops && cipher->ops->set_iv)
		...
		oul_debug_info("cipher", "the %s cipher does not support the set"
						"initialization vector operation\n", cipher->name);
}

void
oul_cipher_context_append(OulCipherContext *context, const guchar *data,
								size_t len)
{
	OulCipher *cipher = NULL;
	...
	if(cipher->ops && cipher->ops->append)
		...
		oul_debug_info("cipher", "the %s cipher does not support the append "
						"operation\n", cipher->name);
}

gboolean
oul_cipher_context_digest(OulCipherContext *context, size_t in_len,
						   guchar digest[], size_t *out_len)
{
	OulCipher *cipher = NULL;
	...
	if(cipher->ops && cipher->ops->digest)
		...
		oul_debug_info("cipher", "the %s cipher does not support the digest "
						"operation\n", cipher->name);
		return FALSE;
	}
}
...
gboolean
oul_cipher_context_digest_to_str(OulCipherContext *context, size_t in_len,
								   gchar digest_s[], size_t *out_len)
{
	...
	if(!oul_cipher_context_digest(context, sizeof(digest), digest, &dlen))
		return FALSE;
...
gint
oul_cipher_context_encrypt(OulCipherContext *context, const guchar data[],
							size_t len, guchar output[], size_t *outlen)
{
	OulCipher *cipher = NULL;
	...
	if(cipher->ops && cipher->ops->encrypt)
		...
		oul_debug_info("cipher", "the %s cipher does not support the encrypt"
						"operation\n", cipher->name);
...
gint
oul_cipher_context_decrypt(OulCipherContext *context, const guchar data[],
							size_t len, guchar output[], size_t *outlen)
{
	OulCipher *cipher = NULL;
	...
	if(cipher->ops && cipher->ops->decrypt)
		...
		oul_debug_info("cipher", "the %s cipher does not support the decrypt"
						"operation\n", cipher->name);
...
void
oul_cipher_context_set_salt(OulCipherContext *context, guchar *salt) {
	OulCipher *cipher = NULL;
...
size_t
oul_cipher_context_get_salt_size(OulCipherContext *context) {
	OulCipher *cipher = NULL;
...
void
oul_cipher_context_set_key(OulCipherContext *context, const guchar *key) {
	OulCipher *cipher = NULL;
...
size_t
oul_cipher_context_get_key_size(OulCipherContext *context) {
	OulCipher *cipher = NULL;
...
void
oul_cipher_context_set_batch_mode(OulCipherContext *context,
                                     OulCipherBatchMode mode)
{
	OulCipher *cipher = NULL;
...
OulCipherBatchMode
oul_cipher_context_get_batch_mode(OulCipherContext *context)
{
	OulCipher *cipher = NULL;
...
size_t
oul_cipher_context_get_block_size(OulCipherContext *context)
{
	OulCipher *cipher = NULL;
...
void
oul_cipher_context_set_key_with_len(OulCipherContext *context,
...
void
oul_cipher_context_set_data(OulCipherContext *context, gpointer data) {
	g_return_if_fail(context);
...
gpointer
oul_cipher_context_get_data(OulCipherContext *context) {
	g_return_val_if_fail(context, NULL);
		...
		const gchar *client_nonce)
{
	...
	oul_cipher_context_append(context, (guchar *)username, strlen(username));
	oul_cipher_context_append(context, (guchar *)":", 1);
	oul_cipher_context_append(context, (guchar *)realm, strlen(realm));
	oul_cipher_context_append(context, (guchar *)":", 1);
	oul_cipher_context_append(context, (guchar *)password, strlen(password));
	...
	if (algorithm != NULL && !g_ascii_strcasecmp(algorithm, "MD5-sess"))
	{
		...
		{
			oul_cipher_context_destroy(context);
			oul_debug_error("cipher", "Required client_nonce missing for MD5-sess digest calculation.\n");
			return NULL;
		}

		oul_cipher_context_digest(context, sizeof(digest), digest, NULL);
		oul_cipher_context_destroy(context);

		context = oul_cipher_context_new(cipher, NULL);
		oul_cipher_context_append(context, digest, sizeof(digest));
		oul_cipher_context_append(context, (guchar *)":", 1);
		oul_cipher_context_append(context, (guchar *)nonce, strlen(nonce));
		oul_cipher_context_append(context, (guchar *)":", 1);
		oul_cipher_context_append(context, (guchar *)client_nonce, strlen(client_nonce));
	}

	oul_cipher_context_digest_to_str(context, sizeof(hash), hash, NULL);
	oul_cipher_context_destroy(context);
		...
		const gchar *session_key)
{
	...
	oul_cipher_context_append(context, (guchar *)method, strlen(method));
	oul_cipher_context_append(context, (guchar *)":", 1);
	oul_cipher_context_append(context, (guchar *)digest_uri, strlen(digest_uri));

	if (qop != NULL && !g_ascii_strcasecmp(qop, "auth-int"))
	{
		OulCipherContext *context2;
		gchar entity_hash[33];

		if (entity == NULL)
		{
			oul_cipher_context_destroy(context);
			oul_debug_error("cipher", "Required entity missing for auth-int digest calculation.\n");
			return NULL;
		}

		context2 = oul_cipher_context_new(cipher, NULL);
		oul_cipher_context_append(context2, (guchar *)entity, strlen(entity));
		oul_cipher_context_digest_to_str(context2, sizeof(entity_hash), entity_hash, NULL);
		oul_cipher_context_destroy(context2);

		oul_cipher_context_append(context, (guchar *)":", 1);
		oul_cipher_context_append(context, (guchar *)entity_hash, strlen(entity_hash));
	}

	oul_cipher_context_digest_to_str(context, sizeof(hash2), hash2, NULL);
	oul_cipher_context_destroy(context);

	context = oul_cipher_context_new(cipher, NULL);
	oul_cipher_context_append(context, (guchar *)session_key, strlen(session_key));
	oul_cipher_context_append(context, (guchar *)":", 1);
	oul_cipher_context_append(context, (guchar *)nonce, strlen(nonce));
	oul_cipher_context_append(context, (guchar *)":", 1);
	...
	if (qop != NULL && *qop != '\0')
	{
		...
		{
			oul_cipher_context_destroy(context);
			oul_debug_error("cipher", "Required nonce_count missing for digest calculation.\n");
			return NULL;
		}
		...
		{
			oul_cipher_context_destroy(context);
			oul_debug_error("cipher", "Required client_nonce missing for digest calculation.\n");
			return NULL;
		}

		oul_cipher_context_append(context, (guchar *)nonce_count, strlen(nonce_count));
		oul_cipher_context_append(context, (guchar *)":", 1);
		oul_cipher_context_append(context, (guchar *)client_nonce, strlen(client_nonce));
		oul_cipher_context_append(context, (guchar *)":", 1);

		oul_cipher_context_append(context, (guchar *)qop, strlen(qop));

		oul_cipher_context_append(context, (guchar *)":", 1);
	}

	oul_cipher_context_append(context, (guchar *)hash2, strlen(hash2));
	oul_cipher_context_digest_to_str(context, sizeof(hash2), hash2, NULL);
	oul_cipher_context_destroy(context);
