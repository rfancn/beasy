...
/* 
 * NT LAN Manager is a Microsoft authentication protocol used with the SMB protocol
 * it is used for authentication with Microsoft remote access protocols.
 *
 * please refer to: http://en.wikipedia.org/wiki/NTLM
 */
...
gchar *
oul_ntlm_gen_type1(const gchar *hostname, const gchar *domain)
{
	int hostnamelen;
	int domainlen;
	unsigned char *msg;
	...
	tmp = oul_base64_encode(msg, sizeof(struct type1_message) + hostnamelen + domainlen);
	g_free(msg);
...
guint8 *
oul_ntlm_parse_type2(const gchar *type2, guint32 *flags)
{
	gsize retlen;
	struct type2_message *tmsg;
	static guint8 nonce[8];

	tmsg = (struct type2_message*)oul_base64_decode(type2, &retlen);
...
/*
 * helper function for oul cipher.c
 */
static void
des_ecb_encrypt(const guint8 *plaintext, guint8 *result, const guint8 *key)
{
	...
	cipher = oul_ciphers_find_cipher("des");
	context = oul_cipher_context_new(cipher, NULL);
	oul_cipher_context_set_key(context, key);
	oul_cipher_context_encrypt(context, plaintext, 8, result, &outlen);
...
gchar *
oul_ntlm_gen_type3(const gchar *username, const gchar *passw, const gchar *hostname, const gchar *domain, const guint8 *nonce, guint32 *flags)
{
	char lm_pw[14];
	...
	if (ucs2le != NULL) {
		...
		oul_debug_info("ntlm", "Unable to encode domain in UTF-16LE.\n");
	}
	...
	if (ucs2le != NULL) {
		...
		oul_debug_info("ntlm", "Unable to encode username in UTF-16LE.\n");
	}
	...
	if (ucs2le != NULL) {
		...
		oul_debug_info("ntlm", "Unable to encode hostname in UTF-16LE.\n");
	}
	...
	cipher = oul_ciphers_find_cipher("md4");
	context = oul_cipher_context_new(cipher, NULL);
	oul_cipher_context_append(context, (guint8 *)nt_pw, 2 * lennt);
	oul_cipher_context_digest(context, 21, nt_hpw, NULL);
	oul_cipher_context_destroy(context);
	...
	tmp = oul_base64_encode((guchar *)tmsg, msglen);
	g_free(tmsg);
